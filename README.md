...
* [x] 创建 Spring Boot 项目
* [x] 导入CS2所有饰品数据（入库）
* [x] 和Steam、BUFF、悠悠有品建立连接，获取BUFF、悠悠的饰品ID，进行关联
* [x] 获取各大平台对应的饰品价格信息（策略模式）
* [x] 实现实时涨跌幅监测各大平台
...
* [x] 自动化监控各大平台
  ...
  * [x] 接入付费平台，获取付费代理
  * [x] 以上不好用（还是会限流），尝试使用 冷热分层 + Serverless 云函数
* [x] 冷热分层（区分普通饰品和热门饰品），冷热监控
* [x] Serverless 云函数走不通，所有IP被封禁了
* [ ] 获取国外代理，获取Steam相关饰品价格信息（未完成，国外代理有点麻烦，一直被重置，通的话限流严重）
* [ ] 悠悠有品的TLS指纹匹配
